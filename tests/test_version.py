...
class TestVersionComparison:
    ...
    def test_version_comparison(self):
        ...
        # comparison based on major version differences
        assert Version("1.0.0") < Version("2.0.0")
        # comparison based on minor version differences
...
class TestVersionOrdering:
    """ complete version ordering according to SemVer specification"""
    ...
    def test_semver_example_ordering(self):
        ...
        for i in range(len(versions)):
            for j in range(i + 1, len(versions)):
                assert Version(versions[i]) < Version(versions[j]), \
                    f"{versions[i]} should be < {versions[j]}"
